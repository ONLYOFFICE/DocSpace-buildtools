			...
			parallel {
				stage('Unix') {
					...
					stages {
						...
						stage('Backend') {
							steps {
								sh 'dotnet build -c Release ASC.Web.slnx'
							}
						}
					}
				}
				stage('Windows') {
					...
					stages {
						...
						stage('Backend') {
							steps {
								bat 'dotnet build -c Release ASC.Web.slnx'
							}
						}
					}
				}
			...
			parallel {
				stage('Unix') {
					...
					stages {
						...
						stage('Files') {
							steps {
								sh "git submodule update --progress --init -- products/ASC.Files/Server/DocStore && dotnet build ASC.Web.slnx && cd ${env.WORKSPACE}/products/ASC.Files/Tests/ && dotnet test ASC.Files.Tests.csproj -r linux-x64 -l \"console;verbosity=detailed\""
							}
						}
					}
				}
				stage('Windows') {
					...
					stages {
						...
						stage('Files') {
							steps {
								bat "git submodule update --progress --init -- products\\ASC.Files\\Server\\DocStore && dotnet build ASC.Web.slnx && cd ${env.WORKSPACE}\\products\\ASC.Files\\Tests\\ && dotnet test ASC.Files.Tests.csproj"
							}
						}
					}
				}
