...
REM fix paths
powershell -Command "(gc buildtools\deploy\nginx\sites-enabled\onlyoffice-editor.conf) -replace 'ROOTPATH', '%~dp0..\..\deploy\products\ASC.Files\editor' -replace '\\', '/' | Out-File -encoding ASCII buildtools\deploy\nginx\sites-enabled\onlyoffice-editor.conf"
powershell -Command "(gc buildtools\deploy\nginx\sites-enabled\onlyoffice-login.conf) -replace 'ROOTPATH', '%~dp0..\..\deploy\login' -replace '\\', '/' | Out-File -encoding ASCII buildtools\deploy\nginx\sites-enabled\onlyoffice-login.conf"
powershell -Command "(gc buildtools\deploy\nginx\sites-enabled\onlyoffice-client.conf) -replace 'ROOTPATH', '%~dp0..\..\deploy\client' -replace '\\', '/' | Out-File -encoding ASCII buildtools\deploy\nginx\sites-enabled\onlyoffice-client.conf"
...
if errorlevel 2 call pnpm e2e.test
if errorlevel 1 call pnpm e2e.test:sequential
