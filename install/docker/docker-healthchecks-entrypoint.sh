...
BACKUP_WORKER_HOST=${BACKUP_WORKER_HOST:-"${CONTAINER_PREFIX}backup-worker:${SERVICE_PORT}"}
...
FILES_WORKER_HOST=${FILES_WORKER_HOST:-"${CONTAINER_PREFIX}files-worker:${SERVICE_PORT}"}
...
API_SERVICE_HOST=${API_SERVICE_HOST:-"${CONTAINER_PREFIX}api:${SERVICE_PORT}"}
...
sed -i "s!localhost:5032!${BACKUP_WORKER_HOST}!g" ${PATH_TO_CONF}/appsettings.json
